...
from .model.selfheco import MultiContrastLoss, MultiHeCo, MultiHeCoWithoutContrastive
...
if __name__ == "__main__":
    for seed in SEEDS:
        ...
        for fold, (train_mask, test_mask) in enumerate(kfold.split(mask, labels[mask].cpu().numpy())):
            ...
            model = MultiHeCoWithoutContrastive(
                features1.shape[1],
                features2.shape[1],
                features3.shape[1],
            ...
            for epoch in range(50):
                ...
                loss = criterion(
                    logits[train_mask], cora1.y[train_mask]
                )
            ...
            with torch.no_grad():
                ...
                y_score_full = torch.softmax(logits_full, dim=1).cpu().numpy()
                y_pred_full = torch.argmax(logits_full, dim=1).cpu().numpy()

                embeds_full = model.get_embeds(
                    cora1,
                    cora2,
                    cora3
                ).cpu().numpy()
            ...
            y_score = y_score_full[test_mask]
            embeds_test = embeds_full[test_mask]
