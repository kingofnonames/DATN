...
if __name__ == '__main__':
    for seed in SEEDS:
        ...
        result_path = BASE_DIR / f"BRCA_v5_results_{seed}_baseline_120.txt"
        for fold, (train_mask, test_mask) in enumerate(kfold.split(mask)):
            ...
            with open(result_path, "a") as f:
                f.write(
                    "Fold %d | Precision: %.4f | Recall: %.4f | F1 Score: %.4f | F1 Weighted Score: %.4f | ACC: %.4f | ARI: %.4f | MCC: %.4f | AUC: %.4f | PR AUC: %.4f | DBI: %.4f | SS: %.4f\n" %
                    (
                        fold,
                        p_scores[-1],
                        r_scores[-1],
