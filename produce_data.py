...
if __name__ == '__main__':
    Gene_Expression = sio.loadmat(BASE_DIR / 'data2' / 'BRCA_gene_matrix.mat')['normalize_corr']
    Methy_Expression = sio.loadmat(BASE_DIR / 'data2' / 'BRCA_methy_matrix.mat')['normalize_corr']
    Mirna_Expression = sio.loadmat(BASE_DIR / 'data2' / 'BRCA_mirna_matrix.mat')['normalize_corr']
    edges1 = sp.coo_matrix(Gene_Expression)
    logging.info("Number of edges in Gene adjacency matrix: %s", edges1.data.shape[0])
    file_write_obj = open(BASE_DIR / 'data2' / "edges_gene_brca.csv", 'w+')
    ...
    file_write_obj = open(BASE_DIR / 'data2' / "edges_methy_brca.csv", 'w+')
    ...
    file_write_obj = open(BASE_DIR / 'data2' / "edges_mirna_brca.csv", 'w+')
